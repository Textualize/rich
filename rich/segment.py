    ...
    def _split_cells(cls, segment: "Segment", cut: int) -> Tuple["Segment", "Segment"]:
        ...
        for pos, char in enumerate(text):
            ...
            if cell_pos + char_width > cut:
                # The cut falls before or inside this character
                if cell_pos == cut:
                    # Clean cut right before this character
                    return (
                        _Segment(text[:pos], style, control),
                        _Segment(text[pos:], style, control),
                    )
                else:
                    # Cut falls inside a double-width character; replace with spaces
                    return (
                        _Segment(text[:pos] + " ", style, control),
            ...
            cell_pos += char_width
            if cell_pos == cut:
                # Clean cut right after this character
                return (
                    _Segment(text[: pos + 1], style, control),
                    _Segment(text[pos + 1 :], style, control),
                )

        # Shouldn't reach here if cut < cell_length, but handle gracefully
        return segment, _Segment("", style, control)
