...
EXTENDED_COLORS = {
    color["colorId"]: (color["rgb"]["r"], color["rgb"]["g"], color["rgb"]["b"])
...
def repl(match):
    low, cap = match.group().lower()
    return f"{low}_{cap.lower()}"


new_colors = {}
for col_no, col in enumerate(TERM_COLORS[16:], 16):
    name = col["name"]
    name = capital.sub(repl, name).lower()
    new_colors[name] = col_no
